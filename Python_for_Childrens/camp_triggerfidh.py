...
#    def run_game():
    def run_game():
        ...
        poweraps = Group()
        guns = Group()
        do_liberint(pygame,Block,blocks,ai_settings,screen,'camp.txt', Alien, aliens, blocks, poweraps)
        aiens = Group()
        ...
        while True:
            ...
            for gun in guns.sprites():
                gun.mon = randint(0,2000)
                if gun.mon == 0:
                    alieh = Alien(ai_settings, screen, ship)
                    alieh.rect.x = gun.rect.x
                    alieh.rect.y = gun.rect.y
                    alieh.x = gun.rect.x
                    alieh.y = gun.rect.y
                    blocks.add(alieh)
...
#                         t = 3
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                        ...
                        mouse_x,mouse_y = pygame.mouse.get_pos()
                        print(len(guns))
                        for gun in aliens.sprites():
                            alien = Clone(ai_settings,screen,mouse_x,mouse_y)
                            alien.rect.x = gun.rect.x
                            alien.rect.y = gun.rect.y
                            alien.x = gun.rect.x
                            alien.y = gun.rect.y
                            bullets.add(alien)
                            blocks.add(alien)
...
#276
 ...
 #           pygame.display.flip()
            ...
            bullets.update(mouse_x, mouse_y)
            ship.blitme()
