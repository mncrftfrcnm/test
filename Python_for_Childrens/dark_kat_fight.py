...
try:
    ...
    class Clone(Sprite):
        def __init__(self,ai_settings,screen, shipx=0, shipy=0):
...
#    def run_game():
    def run_game():
        ...
        aliens.empty()
        ch = 10
        dark_cat = Alien(ai_settings, screen, ship)
        dark_cat.image = pygame.image.load('supercat.gif')
        ...
        while True:
            ...
            if mon == 0:
                ...
                alieh = Bullet(ai_settings, screen, dark_cat, aliens, bullets, 'l', dark_cat.t)
                if alieh.t == 0:
                    alieh.image = pygame.image.load('sword_up.gif')
                if alieh.t == 1:
                    alieh.image = pygame.image.load('sword_down.gif')
                if alieh.t == 2:
                    alieh.image = pygame.image.load('sword_right.gif')
                if alieh.t == 3:
                    alieh.image = pygame.image.load('sword_left.gif') 
                alieh.rect.x = dark_cat.rect.x
                alieh.rect.y = dark_cat.rect.y
                alieh.x = dark_cat.rect.x
                alieh.y = dark_cat.rect.y
                ...
                alieh = Bullet(ai_settings, screen, dark_cat, aliens, bullets, alien, randint(0,3))
                if alieh.t == 0:
                    alieh.image = pygame.image.load('sword_up.gif')
                if alieh.t == 1:
                    alieh.image = pygame.image.load('sword_down.gif')
            ...
            if mon == 0:
                for s in range(3):
                    print('k')
                    alieh = Bullet(ai_settings, screen, dark_cat, aliens, bullets, 'l', s)
                    if alieh.t == 0:
                        alieh.image = pygame.image.load('sword_up.gif')
                    if alieh.t == 1:
                        alieh.image = pygame.image.load('sword_down.gif')
            ...
            if mon == 1000:
                for x in range(9):
                    alieh = Clone(ai_settings, screen, ship.rect.x, ship.rect.y)
                    alieh.rect.x = dark_cat.rect.x
                    alieh.x = dark_cat.rect.x
                    ...
                    alieh.speed_factor = 0.1
                    alieh.image = pygame.image.load('supercat.gif')
                    blocks.add(alieh)
            ...
            if mon == 100:
                for x in range(1):
                    ...
                    aliens.add(alieh)
                dark_cat.rect.x = randint(0, ai_settings.screen_width)
                dark_cat.x = randint(0, ai_settings.screen_width)
                dark_cat.rect.y = randint(0, ai_settings.screen_height)
                dark_cat.y = randint(0, ai_settings.screen_height)
            mon = randint(0,10000)
            if mon == 1000:
                ch += 10
                print(ch)
 ...
 #                       ships2.empty()
                    ...
                    if event.key == pygame.K_3:
                        ...
                        if rod_2 == rod_2 and sg-3 >= 0:
                            new_bullet = Bullet(ai_settings,screen,ship,aliens,bullets,alien,t)
                            sg -= 3
                            new_bullet.count = 10000
                            new_bullet.speed_factor = 0
                            new_bullet.x -= 15
                            new_bullet.rect.x -= 15
                            new_bullet.y -= 3
                            new_bullet.rect.y -= 3
                            
                            new_bullet.image = pygame.image.load('circle.gif')
                            bullets.add(new_bullet)
...
#276
 ...
 #           pygame.display.flip()
            ...
            for bullet in bullets.sprites():
                if bullet.count <= 0:
                    bullets.remove(bullet)
            dark_cat.update(ship.rect.x, ship.rect.y)
            # dark_cat.update(ship.rect.x, ship.rect.y)
            # dark_cat.update(ship.rect.x, ship.rect.y)
            dark_cat.blitme()
            ...
            if pygame.sprite.spritecollideany(dark_cat, bullets):
    
                ch -= 1
                print(ch)
                if ch == 50:
                    dark_cat.image = pygame.image.load('unvisible.gif')
                dark_cat.rect.x = randint(0, ai_settings.screen_width)
                dark_cat.x = randint(0, ai_settings.screen_width)
                dark_cat.rect.y = randint(0, ai_settings.screen_height)
                dark_cat.y = randint(0, ai_settings.screen_height)
