...
class GameStats():
    ...
    def run_game():
        ...
        ship.rect.x = 1112
        ship.rect.y = 668
        tre = Ship(ai_settings,screen)

        tre.rect.x = randint(0,ai_settings.screen_width)
        tre.rect.y = randint(0,ai_settings.screen_height)
        tre.image = pygame.image.load('supercat.gif')
        ships = Group()
        ships.add(ship)
        ships.add(ship2)
    ...
    #276         print(len(alienbullets))
                ...
                ship2.blitme()
                tre.blitme()
                if pygame.sprite.spritecollideany(tre,ships):
                    tre.rect.x = randint(0,ai_settings.screen_width)
                    tre.rect.y = randint(0,ai_settings.screen_height)
