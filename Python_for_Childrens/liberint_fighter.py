try:
    ...
    def run_game():
        ...
        pet.image = pygame.image.load('pet.gif')
        updatenow = 0
        poweraps = Group()
        die_blocks = Group()
            ...
            #for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_w:
                        bullet = Bullet(ai_settings,screen,ship,'','',alien)
                    if event.key == pygame.K_2:
                        images = ['block.gif','robot1.gif','mangal.gif','supercat.gif']
                        print(images)
                        tre = choice(images)

                        ship.image = pygame.image.load(tre)
                    if event.key == pygame.K_DOWN:
...
#276
 ...
 #           pygame.display.flip()
            ...
            pygame.display.flip()
            pygame.time.wait(first)
            if randint(-10000000,10000000000) == 0:
                first = randint(0,50)
                second = randint(0,220)
                three = randint(0,220)
